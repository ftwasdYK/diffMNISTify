...
NN_MODEL = FeatureExtractor(model=resnet50(weights=ResNet50_Weights.DEFAULT), num_layers=NUM_LAYERS_EXPORT).to(DEVICE)

print('Started')
for i, (x, y) in enumerate(train_loader):
    feature_x = NN_MODEL(x.to(DEVICE))
    y = y.numpy()
    np.save(f'data/precompute/train/x{NUM_LAYERS_EXPORT}_{i}.npy', feature_x.cpu().numpy())
    np.save(f'data/precompute/train/y{NUM_LAYERS_EXPORT}_{i}.npy', y)
print('Train features computed.')
for i, (x, y) in enumerate(val_loader):
    feature_x = NN_MODEL(x.to(DEVICE))
    y = y.numpy()
    np.save(f'data/precompute/val/x{NUM_LAYERS_EXPORT}_{i}.npy', feature_x.cpu().numpy())
...
for i, (x, y) in enumerate(test_loader):
    ...
    feature_x = NN_MODEL(x.to(DEVICE))
    y = y.numpy()
    np.save(f'data/precompute/test/x{NUM_LAYERS_EXPORT}_{i}.npy', feature_x.cpu().numpy())
